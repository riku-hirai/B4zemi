...
ary_title = 'shock with S'
ary_header = ['グリッドi', '断面積比A','U1','U2','U3','密度rho', '速度V', '温度T','マッハ数M', '圧力P']
var_title = 'withS'
var_header = ['ステップ数','時間ステップ', '密度rho', '温度T', '圧力P','マッハ数M']
ary_var = np.zeros((mmax+1,6))

def var_array():
    ary_var[0][0] = 0
    ary_var[0][1] = 0
    ary_var[0][2] = copy.copy(ary_0[30][5])
    ary_var[0][3] = copy.copy(ary_0[30][7])
    ary_var[0][4] = copy.copy(ary_0[30][9])
    ary_var[0][5] = copy.copy(ary_0[30][8])
    for  m in range(1,mmax+1):
        ary_var[m][0] = m
        ary_var[m][1] = copy.copy(dt_m[m])
        ary_var[m][2] = copy.copy(ary_m[m][30][5])
        ary_var[m][3] = copy.copy(ary_m[m][30][7])
        ary_var[m][4] = copy.copy(ary_m[m][30][9])
        ary_var[m][5] = copy.copy(ary_m[m][30][8])
...
def calc_flux(array):
    for i in range (0, imax+1):
        ...
        if i ==0:
            ...
            ary_c[m][i][3] = C_x * np.abs(array[i+1][9] - 2 * array[i][9] + array[i-1][9]) / (array[i+1][9] + 2 * array[i][9] + array[i-1][9]) * (array[i+1][2] - 2 * array[i][2] + array[i-1][2])
            ary_c[m][i][4] = C_x * np.abs(array[i+1][9] - 2 * array[i][9] + array[i-1][9]) / (array[i+1][9] + 2 * array[i][9] + array[i-1][9]) * (array[i+1][3] - 2 * array[i][3] + array[i-1][3])
            ary_c[m][i][5] = C_x * np.abs(array[i+1][9] - 2 * array[i][9] + array[i-1][9]) / (array[i+1][9] + 2 * array[i][9] + array[i-1][9]) * (array[i+1][4] - 2 * array[i][4] + array[i-1][4])
...
def calc_S_bar(U1,U2,U3,rho, T):
    ...
    for i in range (0, imax+1):
        ...
        if i == 0:
            ...
            S[0][i] = C_x * np.abs(P[i+1] - 2 * P[i] + P[i-1]) / (P[i+1] + 2 * P[i] + P[i-1]) * (U1[i+1] - 2 * U1[i] + U1[i-1])
            S[1][i] = C_x * np.abs(P[i+1] - 2 * P[i] + P[i-1]) / (P[i+1] + 2 * P[i] + P[i-1]) * (U2[i+1] - 2 * U2[i] + U2[i-1])
            S[2][i] = C_x * np.abs(P[i+1] - 2 * P[i] + P[i-1]) / (P[i+1] + 2 * P[i] + P[i-1]) * (U3[i+1] - 2 * U3[i] + U3[i-1])
...
def calculation(m):
    ...
    for i in range(0, imax+1):
        ...
        if i < imax:
            ...
            rho_bar[i] = rowA[i][2] + rho_pre[i] * dt_m[m] + ary_c[m][i][3]#
            V_bar[i] = rowA[i][3] + V_pre[i] * dt_m[m] + ary_c[m][i][4]#
            T_bar[i] = rowA[i][4] + T_pre[i] * dt_m[m] + ary_c[m][i][5] #
    ...
    S1_bar, S2_bar, S3_bar = calc_S_bar(rho_bar, V_bar, T_bar, rho_shockbar, T_shockbar)
    # print(str(S1_bar))
    # print(str(S2_bar))
    # print(str(S3_bar))
    ...
    for i in range(1, imax):
        ...
        if i == 0:
            ...
            rho_av[i] = 0.5 * (rho_pre[i] + rho_cor[i])
            V_av[i] = 0.5 * (V_pre[i] + V_cor[i])
            T_av[i] = 0.5 * (T_pre[i] + T_cor[i])
            calcA[i][2] = rowA[i][2] + rho_av[i] * dt_m[m] +S1_bar[i]#式合致
            calcA[i][3] = rowA[i][3] + V_av[i] * dt_m[m] +S2_bar[i]#式合致
            calcA[i][4] = rowA[i][4] + T_av[i] * dt_m[m] +S3_bar[i]#式合致
...
csv_make(var_title,var_header,ary_var)