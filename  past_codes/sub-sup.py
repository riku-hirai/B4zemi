...
m = 0
mmax = 1400
ary_0 = np.zeros((imax+1,7)) # row array
ary_m = np.zeros((mmax+1,imax+1,7)) # step array
...
def csv_make(array):
    header = ['グリッドi', '断面積比A', '密度rho', '速度V', '温度T', '圧力P','マッハ数M']
    body = array
    
    with open(outputname(mmax), 'w',newline="") as f:
        writer = csv.writer(f)
...
def calculation(m):
    calcA = np.zeros((imax+1,7))
    rowA  = np.zeros((imax+1,7))
    rho_pre = [0] * (imax+1)
    rho_bar = [0] * (imax+1)
    rho_cor = [0] * (imax+1)
    rho_av = [0] * (imax+1)
